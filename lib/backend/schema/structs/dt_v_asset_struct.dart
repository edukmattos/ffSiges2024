...
class DtVAssetStruct extends FFFirebaseStruct {
  DtVAssetStruct({
    int? id,
    String? code,
    String? description,
  ...
  static DtVAssetStruct fromMap(Map<String, dynamic> data) => DtVAssetStruct(
  ...
  static DtVAssetStruct? maybeFromMap(dynamic data) =>
      data is Map ? DtVAssetStruct.fromMap(data.cast<String, dynamic>()) : null;
  ...
  static DtVAssetStruct fromSerializableMap(Map<String, dynamic> data) =>
      DtVAssetStruct(
        id: deserializeParam(
          data['id'],
          ParamType.int,
  ...
  @override
  String toString() => 'DtVAssetStruct(${toMap()})';

  @override
  bool operator ==(Object other) {
    return other is DtVAssetStruct &&
        id == other.id &&
        code == other.code &&
...
DtVAssetStruct createDtVAssetStruct({
  int? id,
  String? code,
  String? description,
  int? unitId,
...
DtVAssetStruct? updateDtVAssetStruct(
  DtVAssetStruct? dtVAsset, {
  bool clearUnsetFields = true,
  bool create = false,
}) =>
    dtVAsset
...
void addDtVAssetStructData(
  ...
  if (dtVAsset.firestoreUtilData.delete) {
    firestoreData[fieldName] = FieldValue.delete();
    return;
  }
  final clearFields =
      !forFieldValue && dtVAsset.firestoreUtilData.clearUnsetFields;
  if (clearFields) {
    firestoreData[fieldName] = <String, dynamic>{};
  }
  final dtVAssetData = getDtVAssetFirestoreData(dtVAsset, forFieldValue);
  final nestedData = dtVAssetData.map((k, v) => MapEntry('$fieldName.$k', v));

  final mergeFields = dtVAsset.firestoreUtilData.create || clearFields;
  firestoreData
      .addAll(mergeFields ? mergeNestedFields(nestedData) : nestedData);
}

Map<String, dynamic> getDtVAssetFirestoreData(
  DtVAssetStruct? dtVAsset, [
  bool forFieldValue = false,
]) {
  if (dtVAsset == null) {
  ...
  final firestoreData = mapToFirestore(dtVAsset.toMap());

  // Add any Firestore field values
  dtVAsset.firestoreUtilData.fieldValues
      .forEach((k, v) => firestoreData[k] = v);

  return forFieldValue ? mergeNestedFields(firestoreData) : firestoreData;
}

List<Map<String, dynamic>> getDtVAssetListFirestoreData(
  List<DtVAssetStruct>? dtVAssets,
) =>
    dtVAssets?.map((e) => getDtVAssetFirestoreData(e, true)).toList() ?? [];
